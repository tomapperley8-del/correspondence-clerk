/**
 * Gmail Bookmarklet for Correspondence Clerk
 *
 * This script extracts email data from Gmail Web and opens Correspondence Clerk
 ...
 * 4. Click the bookmark while viewing an email in Gmail
 *
...
(function() {
  ...
  function sendToCorrespondenceClerk() {
    ...
    if (!window.location.hostname.includes('mail.google.com')) {
      alert('Please open this bookmarklet while viewing an email in Gmail (mail.google.com).');
      return;
    }

    // Load extractor if not already loaded
    if (!window.extractGmailEmail) {
      loadExtractor();
  ...
  function loadExtractor() {
    ...
    script.src = getCorrespondenceClerkUrl() + '/gmail-extractor.js';
  ...
  function processEmail() {
    try {
      // Extract email data
      const emailData = window.extractGmailEmail();
