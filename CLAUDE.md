...
- **Apr 24, 2026:** UX Audit Phase 10 (P10.1 + P10.2) — P10.1: Actions ItemRow richer layout — business name `text-base font-semibold`, contact name/role `·` separator `text-sm`, subject `italic text-gray-600`, timestamp chip right-aligned ("Received 8d ago" / "Due in 3d" etc.), left border `border-l-[3px]`, row padding `py-3.5`. P10.2: CSS timings — Log/DraftPanel `animate-in fade-in slide-in-from-top-1 duration-150`, CollapsibleSection expand `fade-in duration-150`, ItemRow focus bg instant (removed `transition-colors`). Optimistic mutations — Done and Snooze now remove item immediately and sync in background; `restoreItem()` added to `useActionsData`; `handleSnooze` signature changed to accept `UnifiedItem` (not just id) for rollback support.
