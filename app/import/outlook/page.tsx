...
export default function OutlookImportPage() {
  ...
  useEffect(() => {
    const checkConnection = async () => {
      const res = await fetch('/api/import/outlook/status')
      if (res.ok) {
  ...
  const handleScan = async () => {
    ...
    const res = await fetch('/api/import/outlook/scan', {
      method: 'POST',
    ...
    return (
      <div className="max-w-lg mx-auto mt-12 px-4">
        ...
        <div className="bg-white border border-gray-200 rounded p-8 shadow-[var(--shadow-sm,0_1px_3px_rgba(0,0,0,0.06))]">
          <h1 className="font-[Lora,serif] text-2xl font-semibold text-[#1E293B] mb-2">Connect Outlook</h1>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <h1 className="font-[Lora,serif] text-2xl font-semibold text-[#1E293B] mb-1">Import from Outlook</h1>
      ...
      {step === 'scanning' && (
        <div className="bg-white border border-gray-200 rounded p-8 text-center shadow-[var(--shadow-sm,0_1px_3px_rgba(0,0,0,0.06))]">
          ...
          <p className="text-gray-600 text-sm">Scanning your Outlook inbox…</p>
