...
- [x] **P32** — Actionable insights: contextual action buttons on 7 insight types (Log this call, Copy to clipboard, View Actions, etc.) in expanded InsightCard. Pure UI, zero AI cost. (04/04/2026)
- [x] **P33** — Insight history: `insight_history` table archives every generated insight. "View history" button in expanded InsightCard shows timeline of past snapshots with content preview. Click to view previous version, "Back to current" to return. Migration: `20260407_001_add_insight_history.sql`. (07/04/2026)
- [x] **P34** — Relationship memory: after each business-specific insight generation, Haiku distils a 3-sentence relationship summary and stores it in `businesses.relationship_memory`. Future insights include this memory as context, reducing correspondence limit from 50→30 entries when memory exists. Fire-and-forget — doesn't block insight response. Migration: `20260407_002_add_relationship_memory.sql`. New module: `lib/ai/relationship-memory.ts`. (07/04/2026)
- [x] **P19** — File uploads on business pages: `business_files` table + Supabase Storage bucket `business-files`, server actions (upload/delete/download URL), BusinessFiles component on business page, 10MB per file / 50MB per org cap. Fully deployed — migration applied + bucket created. PDF text extraction deferred. (04/04/2026)
- [x] **P29** — Email import copy review: bookmarklet title/subheading de-jargoned, install step 3 made generic, bulk import banner clarified, scan range hint added to Gmail+Outlook wizards, review step copy clarified, onboarding email-forwarding callout boxes simplified, settings "My email addresses" helper text simplified. (07/04/2026)
